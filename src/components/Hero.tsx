import React from 'react';
import { ArrowRight, BookOpen, Map, ShieldCheck, Mountain } from 'lucide-react';
import { motion } from 'framer-motion';
import { HERO_IMAGE } from '../data/siteImages';

interface HeroProps {
  onReadGuide: () => void;
  onPlanTrip: () => void;
}

export const Hero: React.FC<HeroProps> = ({ onReadGuide, onPlanTrip }) => {
  return (
    <section className="relative min-h-[92vh] sm:min-h-screen flex items-center justify-center overflow-hidden bg-forest-900 text-ivory-100">
      <div className="absolute inset-0 z-0">
        <img
          src={HERO_IMAGE}
          alt="Kainchi Dham ashram beside the rocky Kshipra riverbed, red and saffron shikharas against forested Kumaon hills"
          width={1600}
          height={2000}
          fetchPriority="high"
          decoding="async"
          className="w-full h-full object-cover object-[center_35%]"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-forest-900/88 via-forest-900/50 to-forest-900/25" />
        <div className="absolute inset-0 bg-gradient-to-t from-forest-900 via-transparent to-black/35" />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28 sm:py-36 w-full">
        <div className="max-w-3xl">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="flex flex-wrap items-center gap-2 sm:gap-3 mb-5 sm:mb-6"
          >
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-[0.2em] bg-white/10 text-gold-300 backdrop-blur-md border border-white/15">
              <Mountain className="w-3.5 h-3.5 text-gold-400" />
              Independent visitor guide
            </span>
            <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium tracking-wide bg-forest-800/60 text-ivory-300 backdrop-blur-md border border-forest-600/30">
              Neem Karoli Baba ashram · Nainital district
            </span>
          </motion.div>

          <motion.h1
            initial={{ opacity: 0, y: 25 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.1 }}
            className="font-serif text-4xl sm:text-6xl lg:text-7xl font-light text-white tracking-tight leading-[1.1] mb-6 sm:mb-8"
          >
            Kainchi Dham
            <span className="block text-ivory-300 font-normal italic text-2xl sm:text-4xl lg:text-5xl mt-2 font-serif">
              Booking & Travel Guide 2026
            </span>
          </motion.h1>

          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            className="text-base sm:text-lg lg:text-xl text-ivory-200/90 font-light leading-relaxed max-w-2xl mb-8 sm:mb-10"
          >
            Your trusted independent concierge to Neem Karoli Baba Ashram: daily temple darshan timings, Kathgodam & Delhi taxi booking, verified hotels near temple gate, and Kumaon travel itineraries.
          </motion.p>

          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.3 }}
            className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-10 sm:mb-12"
          >
            <button
              type="button"
              onClick={onReadGuide}
              className="inline-flex items-center justify-center gap-2.5 px-8 py-4 rounded-full bg-ivory-100 text-forest-900 font-semibold text-sm uppercase tracking-wider hover:bg-gold-300 transition-all shadow-elevated"
            >
              <BookOpen className="w-4 h-4" />
              <span>Read the complete guide</span>
              <ArrowRight className="w-4 h-4 text-forest-800" />
            </button>

            <button
              type="button"
              onClick={onPlanTrip}
              className="inline-flex items-center justify-center gap-2.5 px-8 py-4 rounded-full bg-forest-800/80 hover:bg-forest-700/90 text-ivory-100 font-medium text-sm uppercase tracking-wider backdrop-blur-md border border-white/20"
            >
              <Map className="w-4 h-4 text-gold-400" />
              <span>How to reach & itineraries</span>
            </button>
          </motion.div>

          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 1, delay: 0.4 }}
            className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-ivory-300/80 pt-6 border-t border-white/10 font-medium"
          >
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-gold-400" />
              <span>Gates ~6:30 AM–7:30 PM</span>
            </div>
            <span className="hidden sm:inline text-white/20">•</span>
            <span>Kathgodam 37 km</span>
            <span className="hidden sm:inline text-white/20">•</span>
            <span>Free darshan</span>
            <span className="hidden sm:inline text-white/20">•</span>
            <span>Independent travel desk</span>
          </motion.div>
        </div>
      </div>
    </section>
  );
};
